...
def solution(arr):
    answer = -1
    num = int(len(arr)/2) #사람 수
    sub = list(combinations(list(range(num*2)), num)) #팀의 부분집합
    pair = list(combinations(list(range(num)), 2)) #사람 수의 부분집합
    ...
    for i in range(int(len(sub)/2)):
        ...
        for p in pair:
            teamx += arr[x[p[0]]][x[p[1]]] + arr[x[p[1]]][x[p[0]]] 
            teamy += arr[y[p[0]]][y[p[1]]] + arr[y[p[1]]][y[p[0]]] 

        answer = min(abs(teamx-teamy), answer) if answer != -1 else abs(teamx-teamy)
